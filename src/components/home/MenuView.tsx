  ...
  const desktopHome = (
    <div className="desktop-atlas-home hidden min-h-screen md:block">
      ...
      <div className="mx-auto max-w-6xl p-6">
        <div className="mb-6 rounded-lg bg-white p-6 shadow"><div className="mb-4 flex items-center justify-between gap-3"><div><p className="text-xl font-bold text-gray-900">開始今天的學習</p><p className="mt-1 text-sm text-gray-600">{isAllLessonsCompleted ? '全部課程已完成，建議挑戰總測驗' : `下一步：${nextLessonStep?.title ?? '課程'}`}</p></div><p className="text-sm text-gray-600">快速開始，不中斷學習節奏</p></div><div className="grid grid-cols-3 gap-3"><button onClick={() => (isAllLessonsCompleted ? totalQuizStep?.onClick() : nextLessonStep?.onClick())} className="w-full rounded-xl bg-blue-600 px-4 py-3 font-semibold text-white transition-colors hover:bg-blue-700">{isAllLessonsCompleted ? '🎯 挑戰總測驗' : '▶ 繼續學習'}</button><button onClick={onOpenBadgeGallery} className="w-full rounded-xl border border-amber-300 bg-amber-50 px-4 py-3 font-semibold text-amber-800 transition-colors hover:bg-amber-100">查看學習檔案</button><a href="/mailbox" className="rounded-xl border border-[#b48a43] bg-[#f3e8cf] px-4 py-3 text-center font-semibold text-[#765b2d] transition-colors hover:bg-[#fff6df]">真人問答信箱</a></div></div>
