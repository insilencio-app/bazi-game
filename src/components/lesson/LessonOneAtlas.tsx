...
export function LessonOneAtlas({ stage }: { stage: LessonOneAtlasStage }) {
  ...
  if (stage === 'elements') {
    return <div className="rounded-[1.35rem] border border-[#DDD2BF] bg-[#FFFDF7] p-5 shadow-[8px_8px_0_#EEE2CE] sm:p-7"><AtlasHeading number="02" eyebrow="五種方向" title="五行先不是性格，而是動態感。"><p className="mt-4 max-w-none text-[0.98rem] leading-8 text-slate-600 sm:text-[1.05rem]">點擊任一張典籍卡，先以簡潔的方向感認識木、火、土、金、水；你原有的方向、季節與情感資料都保留在此。</p></AtlasHeading><div className="grid grid-cols-5 gap-2 sm:gap-3">{ELEMENTS.map((element) => <button key={element.name} type="button" onClick={() => setSelectedElement(element.name)} className={`min-h-[126px] border p-3 text-left transition-all ${element.tone} ${selected.name === element.name ? 'scale-[1.03] ring-2 ring-[#D9B76E] ring-offset-2 shadow-lg' : 'hover:-translate-y-1 hover:shadow-md'}`}><span className="block font-serif text-3xl font-black sm:text-4xl">{element.name}</span><span className="mt-2 block text-[11px] font-bold tracking-wide opacity-80 sm:text-xs">{element.cue}</span></button>)}</div><div className={`mt-4 grid gap-3 border p-4 sm:grid-cols-[120px_1fr] ${selected.tone}`}><div><span className="font-serif text-5xl font-black">{selected.name}</span><p className="mt-1 text-sm font-semibold opacity-70">{selected.english}</p></div><div className="grid grid-cols-2 gap-2 sm:grid-cols-4">{[['方向', selected.direction], ['季節', selected.season], ['情感', selected.emotion], ['記憶', selected.cue]].map(([label, value]) => <div key={label} className="bg-white/70 p-2.5"><span className="block text-xs font-bold opacity-60">{label}</span><span className="mt-1 block text-[15px] font-bold">{value}</span></div>)}</div></div></div>;
  }
