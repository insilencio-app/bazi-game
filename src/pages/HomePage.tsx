  ...
  if (currentMode === 'menu') {
    return (
      <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100">
        {/* Header */}
        <header className="bg-gradient-to-r from-blue-600 to-indigo-600 text-white p-8 shadow-lg">
          <h1 className="text-6xl font-bold mb-2">輕鬆學八字</h1>
          <p className="text-2xl opacity-90">Learn BaZi in an Interactive Way</p>
        </header>
        ...
        <div className="max-w-6xl mx-auto p-6">
          <div className="grid grid-cols-3 gap-4 mb-8">
            <div className="bg-white p-6 rounded-lg shadow text-center">
              <div className="text-5xl font-bold text-blue-600">{userProgress.lessonsCompleted}</div>
              <p className="text-lg text-gray-600 mt-2">已完成課程</p>
            </div>
            <div className="bg-white p-6 rounded-lg shadow text-center">
              <div className="text-5xl font-bold text-green-600">{userProgress.quizzesCompleted}</div>
              <p className="text-lg text-gray-600 mt-2">已完成測驗</p>
            </div>
            <div className="bg-white p-6 rounded-lg shadow text-center">
              <div className="text-5xl font-bold text-purple-600">{userProgress.totalScore}</div>
              <p className="text-lg text-gray-600 mt-2">總分</p>
            </div>
          </div>
          ...
          <div className="bg-white rounded-3xl shadow-xl p-8 mb-8 relative overflow-hidden">
            ...
            <div className="relative">
              <div className="flex items-center justify-between mb-6">
                <div>
                  <h2 className="text-4xl font-bold text-gray-900">學習路徑</h2>
                  <p className="text-lg text-gray-600 mt-1">跟著路徑完成每個課程步驟</p>
                </div>
                  ...
                  {pathSteps.map((step, index) => (
                    ...
                    >
                      ...
                      <button
                        ...
                        className={`w-full md:w-[70%] text-left p-6 rounded-2xl shadow-lg border border-white/60 bg-gradient-to-br ${step.accent} text-white hover:scale-[1.01] transition-transform`}
                      >
                        <div className="flex items-center justify-between">
                          <span className="text-4xl">{step.emoji}</span>
                          <span className="px-3 py-1 rounded-full text-sm font-semibold bg-white/20">{step.chip}</span>
                        </div>
                        <h3 className="text-3xl font-bold mt-3">{step.title}</h3>
                        <p className="text-white/90 mt-1 text-base">{step.subtitle}</p>
                      </button>
                    </div>
                  ))}
                </div>
          ...
          {selectedElement && (
            <div className="bg-white rounded-lg shadow-lg p-6">
              ...
              <div className="grid grid-cols-2 gap-4 mb-6">
                <div>
                  <p className="text-base text-gray-600">方向</p>
                  <p className="text-3xl font-bold">{selectedElement.direction}</p>
                </div>
                <div>
                  <p className="text-base text-gray-600">季節</p>
                  <p className="text-3xl font-bold">{selectedElement.season}</p>
                </div>
                <div>
                  <p className="text-base text-gray-600">情感</p>
                  <p className="text-3xl font-bold">{selectedElement.emotion}</p>
                </div>
                <div>
                  <p className="text-base text-gray-600">顏色</p>
                  <p className="text-3xl font-bold">{selectedElement.color}</p>
                </div>
              </div>
              <p className="text-xl text-gray-700">{selectedElement.description}</p>
            </div>
          )}
        </div>
      </div>
          ...
          <div className="grid gap-6">
            {mockLessons.map((lesson) => (
              <div key={lesson.id} className="bg-white rounded-lg shadow p-6">
                <h3 className="text-4xl font-bold mb-2">{lesson.title_cn}</h3>
                <p className="text-lg text-gray-600 mb-4">{lesson.title_en}</p>
                <button
      ...
      <div className="min-h-screen bg-gray-50 pb-12">
        ...
        <div className="max-w-7xl mx-auto p-6">
          ...
          <div className="bg-white rounded-lg shadow-lg p-8 mb-8">
            <h2 className="text-2xl font-bold mb-6">十天干 (Heavenly Stems)</h2>
            <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
              {mockHeavenlySteams.map((stem) => (
                <div
                  key={stem.id}
                  className="p-4 bg-blue-100 rounded text-center cursor-pointer hover:bg-blue-200 transition-colors"
                >
                  <p className="text-3xl font-bold">{stem.name_cn}</p>
                  <p className="text-base text-gray-700 mt-1">{stem.name_en}</p>
                  <p className="text-base font-semibold text-blue-700 mt-1">{stem.element}</p>
                  <p className="text-sm text-gray-700 mt-1">{stem.yin_yang === 'yang' ? '陽' : '陰'}</p>
                  <p className="text-sm text-gray-600 mt-1">特質: {stem.personality_traits.join('、')}</p>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="bg-white rounded-lg shadow-lg p-8">
            <h2 className="text-2xl font-bold mb-6">十二地支 (Earthly Branches)</h2>
            <div className="grid grid-cols-3 sm:grid-cols-6 gap-3">
              {mockEarthlyBranches.map((branch) => (
                <div
                  key={branch.id}
                  className="p-4 bg-green-100 rounded text-center cursor-pointer hover:bg-green-200 transition-colors"
                >
                  <p className="text-3xl font-bold">{branch.name_cn}</p>
                  <p className="text-base text-gray-700 mt-1">{branch.zodiac_animal}</p>
                  <p className="text-base font-semibold text-green-700 mt-1">{branch.element}</p>
                  <p className="text-sm text-gray-700 mt-1">{branch.yin_yang === 'yang' ? '陽' : '陰'}</p>
                  <p className="text-sm text-gray-600 mt-1">時辰: {branch.hour_range}</p>
                </div>
              ))}
            </div>
          </div>
